...
//console.log({productTaxonomies, phoneTypes, systemTypes, products, prods});
  ...
  const handleCategoryTabSelect = (tab: React.SetStateAction<number>, taxonomyName?: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | null | undefined) => {
    setActiveTab(tab);
    ...
    if (taxonomyName === '') {
      ...
      switch (tab) {
        case 0:
          content = phoneTypes.map((type: { id: string; attributes: { Image: { data: { attributes: { url: string; }; }; }; Type: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | ReactFragment | ReactPortal | null | undefined; }; }) => (
            // @ts-ignore
            <div key={'phone_' + type.id} className={styles.cardContainer} onClick={handleSelected}>
              <img src={'http://localhost:1337' + type.attributes.Image.data.attributes.url} alt="logo"
                   className={styles.cardImage}/>
